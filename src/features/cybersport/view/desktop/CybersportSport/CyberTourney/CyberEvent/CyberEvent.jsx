...
const CyberEvent = ({ event, addToBasket, selectedRates, addFavoritEvent, isFavorites, oddType }) => {
  ...
  const additionalBet = (<Link
    ...
    to={`cybersport/full-event/line/${event.ID}`}>
  ...
  return (
    <div className={b('wrapper')}>
      <div className={b({ open: isOpen })}>
        <div className={b('title')}>
          <SVGInline
            ...
            svg={isFavorites ? activeStarSvg : starSvg}
            onClick={() => addFavoritEvent(event)} />
          <Link className={b('link')} to={`/cybersport/full-event/line/${event.ID}`}>
      ...
      {isOpen && <FullEventLine eventID={event.ID} inLine />}
    </div>
  );
};
